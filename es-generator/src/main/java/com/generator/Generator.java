...
public class Generator {
    ...
    private static final String URL = "jdbc:mysql://192.168.56.102:3306/es-server?useSSL=false&useUnicode=true&characterEncoding=utf-8&serverTimezone=Asia/Shanghai&useSSL=false";
    private static final String USERNAME = "root";
    private static final String PASSWORD = "admin";
    // TODO：项目根目录（需要修改成自己的）
    private static final String PROJECT_ROOT_PATH = System.getProperty("user.dir");
    //TODO：这里需要注意IOT-admin是项目模块名 如果是多模块模式就需要加上模块名路径
    private static final String BASE_PATH = PROJECT_ROOT_PATH + "/es-generator/src/main/java/com/generator/db";
    private static final String PARENT_PACKAGE_NAME = "com.fengling.db";
    ...
    private static void codeGenerator() {
        ...
        String mapperXmlPath = BASE_PATH + "/mapper/xml";
...
//                        .fileOverride()
                ...
                .strategyConfig((scanner, builder) -> builder.addInclude(getTables(scanner.apply("请输入表名，多个英文逗号分隔？生成所有表，请输入[all]：")))
                        .addTableSuffix("laram_")
                        .addTablePrefix("laram_")
                        // 阶段1：Entity实体类策略配置
                        .entityBuilder()
                        // 设置父类。会在生成的实体类名后：extends BaseEntity
                         .superClass(LavaDo.class)
                        ...
                        // 添加父类公共字段。
                        // 这些字段不会出现在新增的实体类中。
//                        .addSuperEntityColumns("id", "gmt_create", "gmt_modified", "creator", "modifier", "is_deleted")
                        // 添加忽略字段。
                        // 这些字段不会出现在新增的实体类中。
